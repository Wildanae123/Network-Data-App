...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOGS_DIR, 'network_fetcher_dev.log')),
...
class AristaEAPIClient(APIClientBase):
    ...
    def _get_server_connection(self):
        ...
        if self.switch is None:
            try:
                self.switch = Server(self.server_url)
                # Set socket timeout for the underlying connection
...
class ConfigManager:
    ...
    def __init__(self, config_file: str = "commands_dev.yaml"):
...
class DataProcessor:
    ...
    def save_results(self, results, session_id: str = None, selected_commands: List[str] = None):
        ...
        if selected_commands and len(selected_commands) > 0:
            cmd_suffix = f"_{'_'.join(selected_commands[:3])}"  # First 3 commands
...
if __name__ == '__main__':
    ...
    logger.info("Starting Network Data App Development Server")
